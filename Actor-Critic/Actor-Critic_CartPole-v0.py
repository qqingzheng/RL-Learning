...
ac = DModule.Actor_Critic(actor_nn,critic_nn,critic_fixed_nn,device,actor_optim,critic_optim,list(range(2)))
...
for episode in range(EPISODE):
    ...
    while True:
        ...
        td_error = ac.get_error(state).item()
        next_state = torch.tensor(next_state).unsqueeze(0)
        ac.critic_learn(state,action,reward,next_state)
    ...
    if episode % 5 == 0:
        ac.update_critic_fixed_network()
